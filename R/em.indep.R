...
if (nulltype > 0) {
 ...
 BIC<- logL<- (-Inf)
 em.var<-list(ptheta=ptheta.old, f0=f0.old, f1=kern.f1, LIS=lfdr, logL=logL, BIC=BIC, ni=niter, converged=converged)
}
...
if (nulltype > 0) {
 ...
 BIC<- logL<- (-Inf)
 em.var<-list(ptheta=ptheta.old, f0=f0.old, f1=f1.old, LIS=lfdr, logL=logL, BIC=BIC, ni=niter, converged=converged)
}
